...
function define_grouped_permission_checkboxes(id_prefix, which_groups = null) {
  ...
  let group_table = $(`
    <table id="${id_prefix}" class="ui-widget-content" width="100%">
        <tr id="${id_prefix}_header">
            <th id="${id_prefix}_header_p" width="99%">Direct permissions for <span id="${id_prefix}_header_username"></span>
            </th>
  ...
  let update_group_checkboxes = function () {
    ...
    if (
      $("#permdialog_grouped_permissions_Full_control_allow_checkbox").is(
        ":checked"
      )
    ) {
      $("#permdialog_grouped_permissions_Full_control_name").append(
        "<div id='full_control_alert'><span class='oi oi-warning'></span>This action will give user complete access to the folder, including the ability to change permissions on that folder.</div>"
      );
    }
    else {
        $("#full_control_alert").remove();
    }
  };
...
// $('#permdialog_grouped_permissions_Full_control_allow_checkbox').on("change", function(){
//     console.log("hi");
//     $('#permdialog_grouped_permissions_Full_control_name').append("<div>test</div>")
// })
